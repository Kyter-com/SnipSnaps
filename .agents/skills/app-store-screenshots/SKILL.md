...
- **The in-test permission-dialog walker is now the primary path, not a fallback.** After tapping the app's enable button, query `XCUIApplication(bundleIdentifier: "com.apple.springboard").buttons["Allow Full Access"]`, then fall back to any button whose label starts with "Allow". Run once per (not-determined) simulator; subsequent runs see the grant.
